function calculateMonthlyPayment() {
    var lendAmount = parseFloat(document.querySelector('input[name="admin-lend-amount"]:checked')?.value || 0);
	
	var interestRate = parseFloat(document.querySelector('input[name="admin-interest-amount"]:checked')?.value.replace('% Monthly', '') || 0);
	
	var paymentTerm = parseFloat(document.querySelector('input[name="admin-term-amount"]:checked')?.value.replace(' Months', '').replace(' Month', '') || 0);
...
function showLendForm() {
	...
	document.getElementById('adminLendForm').classList.add('active');
	document.getElementById('lendManagerButton').classList.remove('active');
	document.getElementById('adminLendManager').classList.remove('active');
}
...
function showLendManager() {
	...
	document.getElementById('adminLendForm').classList.remove('active');
	document.getElementById('lendManagerButton').classList.add('active');
	document.getElementById('adminLendManager').classList.add('active');
}
...
function validateLendButtons() {
	const lendRadios = document.getElementsByName('admin-lend-amount');
...
function validateInterestButtons() {
	const interestRadios = document.getElementsByName('admin-interest-amount');
...
function validateCollateralButtons() {
	const collateralRadios = document.getElementsByName('admin-collateral-amount');
...
function validateTermButtons() {
	const termRadios = document.getElementsByName('admin-term-amount');
...
function validateLendFormDynamically() {
    const requiredFields = ['admin-lend-amount', 'admin-interest-amount', 'admin-collateral-amount', 'admin-term-amount'];
...
document.addEventListener("DOMContentLoaded", () => {
    const formFields = document.querySelectorAll('#admin-lend-form input');
    const form = document.getElementById('admin-lend-form');
...
$(document).ready(function () {
    ...
    function loadLendingTermsData() {
        ...
        Promise.all([$.ajax({
            url: 'functions/load-admin-lenders-lending-terms-card.php',
            type: 'GET',
            cache: false
        })])
...
$(document).ready(function () {
    ...
    function loadAdminApplicationCard() {
        ...
        $.ajax({
            url: 'functions/load-admin-lenders-application-card.php',
            type: 'GET',
            cache: false,
    ...
    loadAdminApplicationCard();

    setInterval(loadAdminApplicationCard, 30000);
	
	$(window).on('beforeunload', saveScrollPositions);
});

$(document).ready(function () {
    function loadAdminLendManagerNotification() {
        $.ajax({
            ...
            success: function (response) {
                ...
                $('#adminLendManagerNotification').html(response);
            },
            error: function () {
                $('#adminLendManagerNotification').html('<p class="error-message"></p>');
            }
        });
    }

    loadAdminLendManagerNotification();
