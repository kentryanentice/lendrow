...
function pay($connection, $decodedPaymentsId, $decodedApplicationsId, $decodedLenderName, $decodedBorrowerName, $decodedMobile, $decodedMonthly, $userId, $decodedMonth) {
	...
	for ($month = 1; $month <= 11; $month++) {
		...
		if (!isMonthPaid($connection, $decodedPaymentsId, $newMonth)) {
			header("location: ../payments?error=unpaid");
			exit();
		}
	}
    ...
    if ($rowTerm['term'] === '12 Months') {
        $statusUpdate = "UPDATE financial_details SET status = 'Paid', updated_at_month_12 = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE term = '12 Months' AND id = ? AND lendername = ? AND borrowername = ? AND mobile = ? AND applications_id = ?;";
	...
	$paymentUpdate = "UPDATE financial_details SET month_12 = 'Paid', updated_at_month_12 = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND lendername = ? AND borrowername = ? AND mobile = ? AND applications_id = ?;";
