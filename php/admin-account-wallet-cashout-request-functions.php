...
function getCashoutRequest($connection, $searchCashoutRequest = '') {
    $query = "SELECT * FROM cash_transactions";
    $params = [];
    $paramTypes = "";

    if (!empty($searchCashoutRequest)) {
        ...
        $searchTerm = '%' . $searchCashoutRequest . '%';
    ...
    $cashoutRequest = mysqli_fetch_all($resultData, MYSQLI_ASSOC);
