...
function pay($connection, $decodedPaymentsId, $decodedApplicationsId, $decodedLenderName, $decodedBorrowerName, $decodedMobile, $decodedMonthly, $userId, $decodedMonth) {
    ...
    if ($rowTerm['term'] === '10 Months') {
        $statusUpdate = "UPDATE financial_details SET status = 'Paid', updated_at_month_10 = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE term = '10 Months' AND id = ? AND lendername = ? AND borrowername = ? AND mobile = ? AND applications_id = ?;";
	...
	$paymentUpdate = "UPDATE financial_details SET month_10 = 'Paid', updated_at_month_10 = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND lendername = ? AND borrowername = ? AND mobile = ? AND applications_id = ?;";
