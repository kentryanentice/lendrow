...
include 'admin-lenders-application-approval-functions.php';
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["approve"])) {
	...
	if (!ctype_digit($decodedLendingTermsId) || !ctype_digit($decodedApplicationsId) || !ctype_digit($decodedApplicationsUsersId)) {
        header("Location: ../admin-lenders?error=invalidform");
        exit();
    }
	
	if(existingApproved($connection, $decodedApplicationsId, $decodedApplicationsUsersId) !== false) {
		header("location: ../admin-lenders?error=existingapproved");
		exit();
	}

	if(existingDebt($connection, $decodedApplicationsUsersId) !== false) {
		header("location: ../admin-lenders?error=existingdebt");
		exit();
	}

    approve($connection, $adminId, $decodedLenderName, $decodedBorrowerName, $decodedMobile, $decodedAmount, $decodedInterest, $decodedTerm, $decodedMonthly, $decodedCollateral, $decodedLendingTermsId, $decodedApplicationsId, $decodedApplicationsUsersId);
...
elseif ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["disburse"])) {
	...
	disburse($connection, $adminId, $decodedPicture, $decodedLenderName, $decodedBorrowerName, $decodedMobile, $decodedAmount, $decodedInterest, $decodedTerm, $decodedMonthly, $decodedCollateral, $decodedLendingTermsId, $decodedApplicationsId, $decodedApplicationsUsersId, $decodedAgreementsId);
